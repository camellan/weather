        ...
        public Preferences (Weather.MainWindow window, Weather.Widgets.Header header) {
            ...
            boot_sw.notify["active"].connect (() => {
                if (boot_sw.get_active ()) {
                    ...
                    Weather.Utils.set_start_on_boot ();
                } else {
                    setting.set_boolean ("start-on-boot", false);
                    Weather.Utils.reset_start_on_boot ();
                }
            });
            ...
            layout.attach (api_lab, 2, 9, 1, 1);
            layout.attach (api_entry, 3, 9, 1, 1);
            layout.attach (loc_label, 2, 10, 1, 1);
            layout.attach (loc, 3, 10, 1, 1);
